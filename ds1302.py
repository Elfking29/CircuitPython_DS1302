...
class DS1302:
    ...
    def __init__(self,rs_pin:Pin,io_pin:Pin,ck_pin:Pin)->None:
        self.__rs=digitalio.DigitalInOut(rs_pin)
    ...
    def datetime(self)->struct_time:
        ...
        if ((year%4==0 and year%100!=0) or (year%400==0)) and month>2: yday+=1
    ...
    def datetime(self,t:struct_time)->None:
        ...
        if self.__read_reg(0x85)&0x08:
            ...
            if self.clamp(t.tm_hour,0,23)<12: data[2]=data[2]&~0x20
            else: data[2]=data[2]|0x20
        else:
    ...
    def is_12_hour(self,enable_12_hour:bool)->None:
        ...
        if enable_12_hour:
            ...
            if self.clamp(hour,0,23)<12: data=data&~0x20
            else: data=data|0x20
        else:
    ...
    def trickle_enable(self,enable:bool)->None:
        if enable: self.__write_reg(0x90,self.__read_reg(0x91)&~(0x80|0x40|0x20|0x10)|(0x80|0x20))
        else: self.__write_reg(0x90,self.__read_reg(0x91)&~(0x80|0x40|0x20|0x10)|(0x40|0x10))
    ...
    def trickle_resistor(self,resistor:int)->None:
        if resistor in (_RESISTOR_2,_RESISTOR_4,_RESISTOR_8):
            ...
            raise ValueError(f"Resistor value {resistor} is not {_RESISTOR_2}, {_RESISTOR_4}, or {_RESISTOR_8}.")
    ...
    def read_ram(self,offset:int,length:int=1)->bytearray:
        if not 0<=offset<=30: raise ValueError("Starting offset must be between 0 and 30, inclusive.")
        if offset+length>31: raise IndexError(f"Read range {offset+length} exceeds RAM boundary")
        if offset == 0:
    ...
    def read_ram_single(self,offset:int)->int:
        if not 0<=offset<=30: raise ValueError("Offset must be between 0 and 30, inclusive.")
        return self.__read_reg(0xC1+offset*2)
    
    def write_ram(self,offset:int,data:ReadableBuffer)->None:
        if len(data)==0: return
        if not 0<=offset<=30: raise ValueError("Starting offset must be between 0 and 30, inclusive.")
        if offset+len(data)>31: raise IndexError(f"Write range {offset+len(data)} exceeds RAM boundary")
        for value in data:
            if not 0x00<=value<=0xFF: raise ValueError("All data values must be between 0 and 255")
        self.__setwp(False)
        if offset==0:
    ...
    def write_ram_single(self,offset:int,data:int)->None:
        if not 0<=offset<=30: raise ValueError("Offset must be between 0 and 30, inclusive.")
        if not 0x00<=data<=0xFF: raise ValueError("Data must be between 0 and 255, inclusive.")
